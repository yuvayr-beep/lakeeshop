...
export default function ProductTypeModal({ open, onClose, onSuccess, productType }: ProductTypeModalProps) {
  const isEdit = !!productType;

  const [code, setCode] = useState('');
  const [displayName, setDisplayName] = useState('');
  const [namingTemplate, setNamingTemplate] = useState('{brand} {model_name} {capacity} {color}');
  ...
  useEffect(() => {
    if (open) {
      if (productType) {
        ...
        setDisplayName(productType.displayName);
        setNamingTemplate(productType.namingTemplate || '');
      } else {
        setCode('');
        setDisplayName('');
        setNamingTemplate('{brand} {model_name} {capacity} {color}');
      }
    }
  }, [open, productType]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    const toastId = toast.loading(isEdit ? 'Updating product type...' : 'Creating product type...');

    const payload = {
      code: code.trim().toUpperCase().replace(/[^A-Z0-9_-]+/g, '_'),
      displayName: displayName.trim(),
      namingTemplate: namingTemplate.trim() || null,
    };

    try {
      if (isEdit && productType) {
        await axiosInstance.put(`/prod/types/${productType.productTypeId}`, payload);
      } else {
        await axiosInstance.post('/prod/types', payload);
      }
      toast.success(isEdit ? 'Product type updated successfully!' : 'Product type created successfully!', { id: toastId });
      onSuccess();
      onClose();
    } catch (err: any) {
      console.error(err);
      ...
      <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[90vh] border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">
                {isEdit ? `Modifying product type details for ${productType?.code}` : 'Register a new product type master record'}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-4 font-sans">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Smart Phone, Laptops"
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. ELECTRONICS, PHONE1"
        ...
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-150 dark:border-slate-800 flex items-center justify-end gap-3 flex-shrink-0">
          <button
            ...
            disabled={saving}
            onClick={onClose}
            className="px-4 py-2 text-xs font-semibold border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all text-slate-655 dark:text-slate-350 disabled:opacity-50"
          >
            Cancel
          </button>
          <button
