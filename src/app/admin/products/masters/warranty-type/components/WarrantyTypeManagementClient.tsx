...
import WarrantyTypeModal from './WarrantyTypeModal';
...
export default function WarrantyTypeManagementClient() {
  ...
  // Fetch warranty types list (NDJSON stream)
  const fetchWarranties = useCallback(async () => {
    setLoading(true);
    setError('');
    try {
      const { data } = await axiosInstance.get<string>('/prod/warranty/all', {
        headers: { Accept: 'application/x-ndjson' },
        transformResponse: [(data) => data],
      });
      const parsed = parseNdjson(data);
      setWarrantyList(parsed);
    } catch (err: any) {
      console.error(err);
      setError('Failed to load warranty type master records');
      toast.error('Failed to load warranty types list');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const filteredWarranties = useMemo(() => {
    if (!searchQuery.trim()) return warrantyList;
  ...
  const totalPages = Math.max(1, Math.ceil(filteredWarranties.length / itemsPerPage));
  ...
  const startIdx = filteredWarranties.length === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1;
  const endIdx = Math.min(currentPage * itemsPerPage, filteredWarranties.length);
  const totalCount = filteredWarranties.length;

  const handleCreateClick = () => {
    setModalOpen(true);
  };

  const handleDeleteClick = (warranty: WarrantyType) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    const toastId = toast.loading('Deactivating warranty type...');
    try {
      await axiosInstance.delete(`/prod/warranty/${warrantyToDelete.warrantyTypeId}`);
      toast.success('Warranty type deactivated successfully', { id: toastId });
      fetchWarranties();
    } catch (err: any) {
      console.error(err);
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Manage product warranty types, codes, display names, and descriptions.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          */}
          ...
          >
            <Plus size={14} /> Create Warranty Type
          </button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800/80 p-4 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        <div className="relative w-full md:flex-1">
          ...
          <input
            type="text"
            placeholder="Search warranty code, name, description…"
            value={searchQuery}
            onChange={(e) => {
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 text-slate-500">
            ...
            <p className="text-sm font-medium">Fetching warranty type records...</p>
          </div>
        ) : error ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-slate-400 px-4">
            ...
            <p className="text-sm font-medium">No warranty type records found</p>
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[800px]">
              <thead>
                <tr className="border-b border-slate-150 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-955/25">
                  ...
                  <th className="w-48 py-2 px-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 select-none">Warranty Code</th>
                    ...
                    <td className="py-3 px-4 text-center">
                      <div className="flex items-center justify-center">
                        <button
                          ...
                          title="Deactivate Warranty Type"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-905 rounded-2xl shadow-2xl w-full max-w-sm flex flex-col p-6 border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider text-center mb-2">
              Deactivate Warranty Type?
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 text-center mb-6 leading-relaxed">
              Are you sure you want to deactivate warranty type <strong className="text-slate-800 dark:text-white">"{warrantyToDelete?.warrantyCode}"</strong>? This will turn off the status.
            </p>
