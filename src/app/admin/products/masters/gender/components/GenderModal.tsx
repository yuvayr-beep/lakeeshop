...
export default function GenderModal({ open, onClose, onSuccess }: GenderModalProps) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!genderCode.trim()) {
      toast.error('Gender Code is required');
      return;
    }
    if (!displayName.trim()) {
    ...
    setSaving(true);
    const toastId = toast.loading('Creating gender...');

    const payload = {
      genderCode: genderCode.trim().toUpperCase(),
      displayName: displayName.trim(),
      description: description.trim() || null,
    };

    try {
      await axiosInstance.post('/prod/genders', payload);
      toast.success('Gender created successfully!', { id: toastId });
      onSuccess();
      onClose();
    } catch (err: any) {
      console.error(err);
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">
                Register a new gender master record
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-4 font-sans">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
              Gender Code <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              required
