...
import ShipmodeModal from './ShipmodeModal';
...
export default function ShipmodeManagementClient() {
  const [modeList, setModeList] = useState<ShipmentMode[]>([]);
  ...
  const [modeToDelete, setModeToDelete] = useState<ShipmentMode | null>(null);
  ...
  // Fetch shipment modes list (NDJSON stream)
  const fetchModes = useCallback(async () => {
    setLoading(true);
    setError('');
    try {
      const { data } = await axiosInstance.get<string>('/prod/shipment-modes/all', {
      ...
      console.error(err);
      setError('Failed to load shipment mode master records');
      toast.error('Failed to load shipment modes list');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const filteredModes = useMemo(() => {
    if (!searchQuery.trim()) return modeList;
    const q = searchQuery.toLowerCase();
    return modeList.filter(
      (m) =>
        m.modeCode.toLowerCase().includes(q) ||
        m.displayName.toLowerCase().includes(q) ||
        (m.description && m.description.toLowerCase().includes(q))
    );
  }, [modeList, searchQuery]);

  const paginated = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    return filteredModes.slice(start, start + itemsPerPage);
  }, [filteredModes, currentPage, itemsPerPage]);

  const totalPages = Math.max(1, Math.ceil(filteredModes.length / itemsPerPage));
  ...
  const startIdx = filteredModes.length === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1;
  const endIdx = Math.min(currentPage * itemsPerPage, filteredModes.length);
  ...
  const handleDeleteClick = (mode: ShipmentMode) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    const toastId = toast.loading('Deactivating shipment mode...');
    try {
      await axiosInstance.delete(`/prod/shipment-modes/${modeToDelete.shipmentModeId}`);
      toast.success('Shipment mode deactivated successfully', { id: toastId });
      fetchModes();
    } catch (err: any) {
      console.error(err);
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Manage product shipment modes, codes, display names, and descriptions.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          */}
          ...
          >
            <Plus size={14} /> Create Shipment Mode
          </button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800/80 p-4 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        <div className="relative w-full md:flex-1">
          ...
          <input
            type="text"
            placeholder="Search shipment mode code, name, description…"
            value={searchQuery}
            onChange={(e) => {
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 text-slate-500">
            ...
            <p className="text-sm font-medium">Fetching shipment mode records...</p>
          </div>
        ) : error ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-slate-400 px-4">
            ...
            <p className="text-sm font-medium">No shipment mode records found</p>
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[800px]">
              <thead>
                <tr className="border-b border-slate-150 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-955/25">
                  ...
                  <th className="w-48 py-2 px-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 select-none">Mode Code</th>
                    ...
                    <td className="py-3 px-4 text-center">
                      <div className="flex items-center justify-center">
                        <button
                          ...
                          title="Deactivate Shipment Mode"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
        ...
        {/* Pagination Footer */}
        {!loading && !error && filteredModes.length > 0 && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-905 rounded-2xl shadow-2xl w-full max-w-sm flex flex-col p-6 border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider text-center mb-2">
              Deactivate Shipment Mode?
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 text-center mb-6 leading-relaxed">
              Are you sure you want to deactivate shipment mode <strong className="text-slate-800 dark:text-white">"{modeToDelete?.modeCode}"</strong>? This will turn off the status.
            </p>
