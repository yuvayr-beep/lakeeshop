...
import PatternFinishModal from './PatternFinishModal';

interface PatternFinish {
  patternFinishId: number;
  patternCode: string;
  displayName: string;
  description: string;
  isActive: boolean;
...
export default function PatternFinishManagementClient() {
  const [patternList, setPatternList] = useState<PatternFinish[]>([]);
  ...
  const [patternToDelete, setPatternToDelete] = useState<PatternFinish | null>(null);
  ...
  // Fetch pattern finishes list (NDJSON stream)
  const fetchPatterns = useCallback(async () => {
    setLoading(true);
    setError('');
    try {
      const { data } = await axiosInstance.get<string>('/prod/pattern/all', {
        headers: { Accept: 'application/x-ndjson' },
        transformResponse: [(data) => data],
      });
      const parsed = parseNdjson(data);
      setPatternList(parsed);
    } catch (err: any) {
      console.error(err);
      setError('Failed to load pattern finish master records');
      toast.error('Failed to load pattern finishes list');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const filteredPatterns = useMemo(() => {
    if (!searchQuery.trim()) return patternList;
  ...
  const startIdx = filteredPatterns.length === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1;
  const endIdx = Math.min(currentPage * itemsPerPage, filteredPatterns.length);
  ...
  const handleDeleteClick = (pattern: PatternFinish) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    const toastId = toast.loading('Deactivating pattern finish...');
    try {
      await axiosInstance.delete(`/prod/pattern/${patternToDelete.patternFinishId}`);
      toast.success('Pattern finish deactivated successfully', { id: toastId });
      fetchPatterns();
    } catch (err: any) {
      console.error(err);
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
            ...
            Pattern Finish Master
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Manage product pattern finishes, codes, display names, and descriptions.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          */}
          ...
          >
            <Plus size={14} /> Create Pattern Finish
          </button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800/80 p-4 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        <div className="relative w-full md:flex-1">
          ...
          <input
            type="text"
            placeholder="Search pattern code, name, description…"
            value={searchQuery}
            onChange={(e) => {
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 text-slate-500">
            ...
            <p className="text-sm font-medium">Fetching pattern finish records...</p>
          </div>
        ) : error ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-slate-400 px-4">
            ...
            <p className="text-sm font-medium">No pattern finish records found</p>
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[800px]">
              <thead>
                <tr className="border-b border-slate-150 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-950/25">
                  ...
                  <th className="w-48 py-2 px-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 select-none">Pattern Code</th>
                    ...
                    <td className="py-3 px-4 text-center">
                      <div className="flex items-center justify-center">
                        <button
                          ...
                          title="Deactivate Pattern Finish"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-905 rounded-2xl shadow-2xl w-full max-w-sm flex flex-col p-6 border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider text-center mb-2">
              Deactivate Pattern Finish?
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 text-center mb-6 leading-relaxed">
              Are you sure you want to deactivate pattern finish <strong className="text-slate-800 dark:text-white">"{patternToDelete?.patternCode}"</strong>? This will turn off the status.
            </p>
