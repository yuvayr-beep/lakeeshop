...
interface PatternFinishModalProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export default function PatternFinishModal({ open, onClose, onSuccess }: PatternFinishModalProps) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!patternCode.trim()) {
      toast.error('Pattern Code is required');
      return;
    }
    if (!displayName.trim()) {
    ...
    setSaving(true);
    const toastId = toast.loading('Creating pattern finish...');

    const payload = {
      patternCode: patternCode.trim().toUpperCase().replace(/[^A-Z0-9_-]+/g, '_'),
      displayName: displayName.trim(),
      description: description.trim() || null,
    };

    try {
      await axiosInstance.post('/prod/pattern', payload);
      toast.success('Pattern finish created successfully!', { id: toastId });
      onSuccess();
      onClose();
    } catch (err: any) {
      console.error(err);
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider">
                Create Pattern Finish
              </h2>
              <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">
                Register a new product pattern finish master record
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-4 font-sans">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setPatternCode(autoCode);
              }}
              placeholder="e.g. Glossy, Matte Finish"
          ...
          <div>
            ...
            <input
              ...
              required
              value={patternCode}
              onChange={(e) => setPatternCode(e.target.value.toUpperCase().replace(/[^A-Z0-9_-]+/g, '_'))}
              placeholder="e.g. PTY2"
              className="w-full h-10 px-3.5 text-sm bg-slate-50/50 focus:bg-white dark:bg-slate-950/20 dark:focus:bg-slate-900 border border-slate-200 hover:border-slate-300 focus:border-blue-500 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 text-slate-800 dark:text-slate-100 placeholder:text-slate-400 transition-all font-medium"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Pattern details and surface characteristics..."
              rows={3}
