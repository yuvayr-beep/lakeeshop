...
export default function ColorModal({ open, onClose, onSuccess, color }: ColorModalProps) {
  const isEdit = !!color;
  ...
  useEffect(() => {
    if (open) {
      if (color) {
        ...
        setDisplayName(color.displayName);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    const toastId = toast.loading(isEdit ? 'Updating color...' : 'Creating color...');
    ...
    if (isEdit && color) {
      payload.colorId = color.colorId;
      payload.isActive = color.isActive;
    }

    try {
      await axiosInstance.post('/prod/colors', payload);
      toast.success(isEdit ? 'Color updated successfully!' : 'Color created successfully!', { id: toastId });
      onSuccess();
      onClose();
    } catch (err: any) {
      console.error(err);
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">
                {isEdit ? `Modifying color details for ${color?.colorCode}` : 'Register a new color master record'}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-4 font-sans">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <input
                type="color"
                value={hexCode.startsWith('#') && hexCode.length === 7 ? hexCode : '#000000'}
                onChange={(e) => setHexCode(e.target.value.toUpperCase())}
                className="w-10 h-10 rounded-xl cursor-pointer border border-slate-200 dark:border-slate-800 p-0.5 bg-transparent overflow-hidden"
              />
              <input
                type="text"
                required
