...
import { 
  Plus, Edit3, Trash2, Search, Loader2, 
  ChevronLeft, ChevronRight, FolderOpen, AlertCircle, RefreshCw, Palette
} from 'lucide-react';
...
export default function ColorManagementClient() {
  ...
  // Fetch colors list (NDJSON stream)
  const fetchColors = useCallback(async () => {
    setLoading(true);
    setError('');
    try {
      const { data } = await axiosInstance.get<string>('/prod/colors/all', {
      ...
      console.error(err);
      setError('Failed to load color master records');
  ...
  const filteredColors = useMemo(() => {
    if (!searchQuery.trim()) return colorList;
    const q = searchQuery.toLowerCase();
    return colorList.filter(
      (c) =>
        c.colorCode.toLowerCase().includes(q) ||
        c.displayName.toLowerCase().includes(q) ||
        (c.hexCode && c.hexCode.toLowerCase().includes(q))
    );
  }, [colorList, searchQuery]);
  ...
  const totalPages = Math.max(1, Math.ceil(filteredColors.length / itemsPerPage));
  ...
  const startIdx = filteredColors.length === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1;
  const endIdx = Math.min(currentPage * itemsPerPage, filteredColors.length);
  ...
  const handleConfirmDelete = async () => {
    ...
    const toastId = toast.loading('Deactivating color...');
    try {
      await axiosInstance.delete(`/prod/colors/${colorToDelete.colorId}`);
      toast.success('Color deactivated successfully', { id: toastId });
      fetchColors();
    } catch (err: any) {
      console.error(err);
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Manage product colors, define hex values, and update display names.
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800/80 p-4 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        <div className="relative w-full md:flex-1">
          ...
          <input
            type="text"
            placeholder="Search color code, name, hex value…"
            value={searchQuery}
            onChange={(e) => {
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 text-slate-500">
            ...
            <p className="text-sm font-medium">Fetching color master records...</p>
          </div>
        ) : error ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-slate-400 px-4">
            ...
            <p className="text-sm font-medium">No color records found</p>
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[700px]">
              <thead>
                <tr className="border-b border-slate-150 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-950/25">
                  ...
                  <th className="w-48 py-2 px-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 select-none">Color Code</th>
                  <th className="py-2 px-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 select-none">Display Name</th>
                  <th className="w-48 py-2 px-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 select-none">Hex Value</th>
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-xs font-medium text-slate-600 dark:text-slate-400">
                      <div className="flex items-center gap-2">
                        <span 
                          className="w-4 h-4 rounded-full border border-slate-200 dark:border-slate-700 shadow-sm flex-shrink-0" 
                          style={{ backgroundColor: color.hexCode || '#000000' }} 
                        />
                        <span className="font-mono uppercase tabular-nums">{color.hexCode || '-'}</span>
                      </div>
                    </td>
                    ...
                    <td className="py-3 px-4 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        {/* Commented out edit button as requested */}
                        {/*
                        <button
      ...
      {deleteConfirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm flex flex-col p-6 border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200">
            <div className="w-12 h-12 rounded-full bg-red-100 dark:bg-red-950/30 flex items-center justify-center text-red-650 dark:text-red-400 mb-4 mx-auto">
              <AlertCircle size={24} />
            </div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 text-center mb-6 leading-relaxed">
              Are you sure you want to deactivate color <strong className="text-slate-800 dark:text-white">"{colorToDelete?.colorCode}"</strong> ({colorToDelete?.displayName})? This will turn off the color status.
            </p>
            <div className="flex items-center justify-end gap-3 font-sans">
              <button
                type="button"
                onClick={() => setDeleteConfirmOpen(false)}
                className="px-4 py-2 text-xs font-semibold border border-slate-200 dark:border-slate-705 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all text-slate-650 dark:text-slate-350"
              >
                Cancel
              </button>
              <button
