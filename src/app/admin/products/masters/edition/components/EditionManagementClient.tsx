...
export default function EditionManagementClient() {
  ...
  // Fetch editions list (NDJSON stream)
  const fetchEditions = useCallback(async () => {
    setLoading(true);
    setError('');
    try {
      const { data } = await axiosInstance.get<string>('/prod/editions/all', {
        headers: { Accept: 'application/x-ndjson' },
        transformResponse: [(data) => data],
      });
      const parsed = parseNdjson(data);
      setEditionList(parsed);
    } catch (err: any) {
      console.error(err);
      setError('Failed to load edition master records');
  ...
  const filteredEditions = useMemo(() => {
    if (!searchQuery.trim()) return editionList;
    const q = searchQuery.toLowerCase();
    return editionList.filter(
      (e) =>
        e.editionCode.toLowerCase().includes(q) ||
        e.displayName.toLowerCase().includes(q) ||
        (e.description && e.description.toLowerCase().includes(q))
    );
  }, [editionList, searchQuery]);

  const paginated = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    return filteredEditions.slice(start, start + itemsPerPage);
  }, [filteredEditions, currentPage, itemsPerPage]);

  const totalPages = Math.max(1, Math.ceil(filteredEditions.length / itemsPerPage));
  ...
  const startIdx = filteredEditions.length === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1;
  const endIdx = Math.min(currentPage * itemsPerPage, filteredEditions.length);
  const totalCount = filteredEditions.length;
  ...
  const handleConfirmDelete = async () => {
    ...
    const toastId = toast.loading('Deactivating edition...');
    try {
      await axiosInstance.delete(`/prod/editions/${editionToDelete.editionId}`);
      toast.success('Edition deactivated successfully', { id: toastId });
      fetchEditions();
    } catch (err: any) {
      console.error(err);
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Manage product editions, define display names, and descriptions.
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800/80 p-4 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        <div className="relative w-full md:flex-1">
          ...
          <input
            type="text"
            placeholder="Search edition code, name, description…"
            value={searchQuery}
            onChange={(e) => {
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 text-slate-500">
            ...
            <p className="text-sm font-medium">Fetching edition master records...</p>
          </div>
        ) : error ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-slate-400 px-4">
            ...
            <p className="text-sm font-medium">No edition records found</p>
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[700px]">
              <thead>
                <tr className="border-b border-slate-150 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-950/25">
                  ...
                  <th className="w-48 py-2 px-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 select-none">Edition Code</th>
        ...
        {/* Pagination Footer */}
        {!loading && !error && filteredEditions.length > 0 && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm flex flex-col p-6 border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 text-center mb-6 leading-relaxed">
              Are you sure you want to deactivate edition <strong className="text-slate-800 dark:text-white">"{editionToDelete?.editionCode}"</strong> ({editionToDelete?.displayName})? This will turn off the edition status.
            </p>
