...
export default function EditionModal({ open, onClose, onSuccess, edition }: EditionModalProps) {
  ...
  useEffect(() => {
    if (open) {
      if (edition) {
        setEditionCode(edition.editionCode);
        setDisplayName(edition.displayName);
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editionCode.trim()) {
      toast.error('Edition Code is required');
      return;
    }
    if (!displayName.trim()) {
    ...
    setSaving(true);
    const toastId = toast.loading(isEdit ? 'Updating edition...' : 'Creating edition...');

    const payload: any = {
      editionCode: editionCode.trim().toUpperCase(),
      displayName: displayName.trim(),
      description: description.trim() || null,
    };

    if (isEdit && edition) {
      payload.editionId = edition.editionId;
      payload.isActive = edition.isActive;
    }

    try {
      await axiosInstance.post('/prod/editions', payload);
      toast.success(isEdit ? 'Edition updated successfully!' : 'Edition created successfully!', { id: toastId });
      onSuccess();
      onClose();
    } catch (err: any) {
      console.error(err);
      ...
      <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[90vh] border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider">
                {isEdit ? 'Edit Edition' : 'Create Edition'}
              </h2>
              <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">
                {isEdit ? `Modifying edition details for ${edition?.editionCode}` : 'Register a new edition master record'}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-4 font-sans">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
              Edition Code <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              required
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Standard Edition, Special Edition"
        ...
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-150 dark:border-slate-800 flex items-center justify-end gap-3 flex-shrink-0">
          <button
            ...
            disabled={saving}
            onClick={onClose}
            className="px-4 py-2 text-xs font-semibold border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all text-slate-650 dark:text-slate-350 disabled:opacity-50"
          >
            Cancel
          </button>
          <button
