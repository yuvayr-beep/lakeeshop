...
import HandlingTypeModal from './HandlingTypeModal';
...
export default function HandlingTypeManagementClient() {
  const [handlingList, setHandlingList] = useState<HandlingType[]>([]);
  ...
  const [handlingToDelete, setHandlingToDelete] = useState<HandlingType | null>(null);
  ...
  // Fetch handling types list (NDJSON stream)
  const fetchHandlingTypes = useCallback(async () => {
    setLoading(true);
    setError('');
    try {
      const { data } = await axiosInstance.get<string>('/prod/handling-type/all', {
        headers: { Accept: 'application/x-ndjson' },
        transformResponse: [(data) => data],
      });
      const parsed = parseNdjson(data);
      setHandlingList(parsed);
    } catch (err: any) {
      console.error(err);
      setError('Failed to load handling type master records');
      toast.error('Failed to load handling types list');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const filteredHandling = useMemo(() => {
    if (!searchQuery.trim()) return handlingList;
    const q = searchQuery.toLowerCase();
    return handlingList.filter(
      (h) =>
        h.handlingCode.toLowerCase().includes(q) ||
        h.displayName.toLowerCase().includes(q) ||
        (h.description && h.description.toLowerCase().includes(q))
    );
  }, [handlingList, searchQuery]);

  const paginated = useMemo(() => {
    const start = (currentPage - 1) * itemsPerPage;
    return filteredHandling.slice(start, start + itemsPerPage);
  }, [filteredHandling, currentPage, itemsPerPage]);

  const totalPages = Math.max(1, Math.ceil(filteredHandling.length / itemsPerPage));
  ...
  const startIdx = filteredHandling.length === 0 ? 0 : (currentPage - 1) * itemsPerPage + 1;
  const endIdx = Math.min(currentPage * itemsPerPage, filteredHandling.length);
  const totalCount = filteredHandling.length;

  const handleCreateClick = () => {
    setModalOpen(true);
  };

  const handleDeleteClick = (handling: HandlingType) => {
    setHandlingToDelete(handling);
    setDeleteConfirmOpen(true);
  };

  const handleConfirmDelete = async () => {
    if (!handlingToDelete) return;
    setDeleteConfirmOpen(false);
    const toastId = toast.loading('Deactivating handling type...');
    try {
      await axiosInstance.delete(`/prod/handling-type/${handlingToDelete.handlingTypeId}`);
      toast.success('Handling type deactivated successfully', { id: toastId });
      fetchHandlingTypes();
    } catch (err: any) {
      console.error(err);
      toast.error('Deactivation failed', { id: toastId });
    } finally {
      setHandlingToDelete(null);
    }
  };
    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
            ...
            Handling Type Master
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Manage product handling types, codes, display names, and descriptions.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button className="flex items-center gap-2 px-3 py-2 border border-slate-200 dark:border-slate-855 bg-white dark:bg-slate-900 rounded-xl text-slate-700 dark:text-slate-350 text-xs font-semibold shadow-sm">
            Upload
          </button>
          */}
          <button
          ...
          >
            <Plus size={14} /> Create Handling Type
          </button>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800/80 p-4 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        <div className="relative w-full md:flex-1">
          ...
          <input
            type="text"
            placeholder="Search handling type code, name, description…"
            value={searchQuery}
            onChange={(e) => {
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 text-slate-500">
            ...
            <p className="text-sm font-medium">Fetching handling type records...</p>
          </div>
        ) : error ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-slate-400 px-4">
            ...
            <p className="text-sm font-medium">No handling type records found</p>
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[800px]">
              <thead>
                <tr className="border-b border-slate-150 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-955/25">
                  ...
                  <th className="w-48 py-2 px-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 select-none">Handling Code</th>
                    ...
                    <td className="py-3 px-4 text-center">
                      <div className="flex items-center justify-center">
                        <button
                          ...
                          title="Deactivate Handling Type"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
        ...
        {/* Pagination Footer */}
        {!loading && !error && filteredHandling.length > 0 && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-905 rounded-2xl shadow-2xl w-full max-w-sm flex flex-col p-6 border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider text-center mb-2">
              Deactivate Handling Type?
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 text-center mb-6 leading-relaxed">
              Are you sure you want to deactivate handling type <strong className="text-slate-800 dark:text-white">"{handlingToDelete?.handlingCode}"</strong>? This will turn off the status.
            </p>
