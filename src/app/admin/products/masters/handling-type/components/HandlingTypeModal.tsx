...
interface HandlingTypeModalProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export default function HandlingTypeModal({ open, onClose, onSuccess }: HandlingTypeModalProps) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!handlingCode.trim()) {
      toast.error('Handling Code is required');
      return;
    }
    if (!displayName.trim()) {
    ...
    setSaving(true);
    const toastId = toast.loading('Creating handling type...');

    const payload = {
      handlingCode: handlingCode.trim().toUpperCase().replace(/[^A-Z0-9_-]+/g, '_'),
      displayName: displayName.trim(),
      description: description.trim() || null,
    };

    try {
      await axiosInstance.post('/prod/handling-type', payload);
      toast.success('Handling type created successfully!', { id: toastId });
      onSuccess();
      onClose();
    } catch (err: any) {
      console.error(err);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[90vh] border border-slate-200 dark:border-slate-800 animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-0.5">
                Register a new product handling type master record
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-6 space-y-4 font-sans">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                setHandlingCode(autoCode);
              }}
              placeholder="e.g. Fragile, Heavy Cargo"
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">
              Handling Code <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              required
              value={handlingCode}
              onChange={(e) => setHandlingCode(e.target.value.toUpperCase().replace(/[^A-Z0-9_-]+/g, '_'))}
              placeholder="e.g. TESTH3"
              className="w-full h-10 px-3.5 text-sm bg-slate-50/50 focus:bg-white dark:bg-slate-955/20 dark:focus:bg-slate-900 border border-slate-200 hover:border-slate-300 focus:border-blue-500 dark:border-slate-800 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 text-slate-800 dark:text-slate-100 placeholder:text-slate-400 transition-all font-medium"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Handling instructions and restrictions..."
              rows={3}
