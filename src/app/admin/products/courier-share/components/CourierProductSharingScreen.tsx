...
import CourierProductShareModal from './CourierProductShareModal';
...
interface CourierProductSharingScreenProps {
  client: Client;
  onBack: () => void;
  clientsList: Client[];
}
...
export default function CourierProductSharingScreen({
  client,
  onBack,
  clientsList,
}: CourierProductSharingScreenProps) {
  ...
  const fetchShares = useCallback(async () => {
    ...
    try {
      ...
      if (isFiltered) {
        url = `/prod/supplier-product-share?supplierId=${client.id}&shareStatus=${subTab}`;
        if (appliedSku.trim() !== '') {
        ...
        url = `/prod/supplier-product-share/paginated?supplierId=${client.id}&shareStatus=${subTab}&page=${page}&size=${size}`;
      }

      const { data } = await axiosInstance.get<any>(url, {
        headers: { Accept: 'application/x-ndjson' },
      });
      const parsed = parseNdjson(data).map((item: any) => ({
        ...item,
        clientShareId: item.id || item.clientShareId,
        minQty: item.moq ?? item.minQty,
        clientSkuCode: item.supplierSkuCode || item.clientSkuCode,
        sellingPrice: item.supplierPrice ?? item.sellingPrice,
        isPreferred: item.isPreferred ?? item.isListed ?? true,
  ...
  const fetchPriceHistory = useCallback(async () => {
    ...
    try {
      let url = `/prod/supplier-product-share/price-history?supplierId=${client.id}&page=${historyPage}&size=${historySize}`;
      ...
      const parsed = parseNdjson(data).map((item: any) => ({
        ...item,
        oldSellingPrice: item.oldSupplierPrice ?? item.oldSellingPrice,
        newSellingPrice: item.newSupplierPrice ?? item.newSellingPrice,
      }));
  ...
  const fetchAllClientShares = useCallback(async () => {
    try {
      const url = `/prod/supplier-product-share?supplierId=${client.id}`;
      const { data } = await axiosInstance.get<any>(url, {
        headers: { Accept: 'application/x-ndjson' },
      });
      const parsed = parseNdjson(data).map((item: any) => ({
      ...
      console.error('Failed to load supplier shares for dropdown', err);
    }
  }, [client.id]);
  ...
  const handleSingleApprove = async (shareId: number) => {
    ...
    try {
      await axiosInstance.post('/prod/supplier-product-share/approve', [shareId]);
  ...
  const handleBulkApprove = async () => {
    ...
    try {
      await axiosInstance.post('/prod/supplier-product-share/approve', selectedIds);
  ...
  // Helper: Construct block item payload
  const getBlockItemPayload = (shareId: number, blockVal: boolean, reasonVal: string) => {
    const shareItem = shares.find(s => s.clientShareId === shareId) || allClientShares.find(s => s.clientShareId === shareId);
    const sku = shareItem?.supplierSkuCode || shareItem?.clientSkuCode || skuCache[shareItem?.skuId] || '';
    return {
      supplierShareId: shareId,
  ...
  const handleBlockSubmit = async (reason: string) => {
    ...
    if (blockingTarget === 'single' && singleBlockId !== null) {
      payload = [getBlockItemPayload(singleBlockId, true, reason)];
    } else {
      payload = selectedIds.map((id) => getBlockItemPayload(id, true, reason));
    }

    try {
      await axiosInstance.post('/prod/supplier-product-share/block', payload);
  ...
  const handleUnblock = async (shareIds: number[]) => {
    ...
    const payload = shareIds.map((id) => getBlockItemPayload(id, false, 'Unblocked'));
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await Promise.all(idsToDelete.map((id) => axiosInstance.delete(`/prod/supplier-product-share/${id}`)));
  ...
  const handleDownloadSample = async () => {
    ...
    try {
      const endpoint = uploadMode === 'add'
        ? '/prod/supplier-product-share/addition-template'
        : uploadMode === 'approve'
          ? '/prod/supplier-product-share/approve/template'
          : uploadMode === 'priceChange'
            ? '/prod/supplier-product-share/price-change/template'
            : '/prod/supplier-product-share/block/template';
      ...
      const filename = uploadMode === 'add'
        ? `supplier_share_addition_template_${Date.now()}.xlsx`
        : uploadMode === 'approve'
          ? `supplier_share_approve_template_${Date.now()}.xlsx`
          : uploadMode === 'priceChange'
            ? `supplier_share_price_change_template_${Date.now()}.xlsx`
            : `supplier_share_block_template_${Date.now()}.xlsx`;
  ...
  const handleUploadExcel = async () => {
    ...
    const endpoint = uploadMode === 'add'
      ? '/prod/supplier-product-share/upload'
      : uploadMode === 'approve'
        ? '/prod/supplier-product-share/approve/upload'
        : uploadMode === 'priceChange'
          ? '/prod/supplier-product-share/price-change/upload'
          : '/prod/supplier-product-share/block/upload';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400 dark:text-slate-505 font-medium">Supplier Product Sharing</span>
      ...
      {mainTab === 'shared' ? (
        <div className="space-y-4">
              ...
              {(subTab === 'DRAFT' || subTab === 'APPROVED') && (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-2 text-xs font-bold rounded-xl shadow-sm transition-all ${showUploadArea && uploadMode === 'block'
                    ...
                    : 'bg-amber-50 hover:bg-amber-100 dark:bg-amber-955/20 dark:hover:bg-amber-900/30 text-amber-600 dark:text-amber-400 border border-amber-250 dark:border-amber-900/50'
                    }`}
                >
                  <Upload size={13} />
                  Block
              ...
              {subTab === 'BLOCKED' && (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-2 text-xs font-bold rounded-xl shadow-sm transition-all ${showUploadArea && uploadMode === 'unblock'
                    ...
                    : 'bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-955/20 dark:hover:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 border border-emerald-250 dark:border-emerald-900/50'
                    }`}
                >
                  <Upload size={13} />
                  Unblock
              ...
              {(subTab === 'DRAFT' || subTab === 'BLOCKED') && (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-2 text-xs font-bold rounded-xl shadow-sm transition-all ${showUploadArea && uploadMode === 'approve'
                    ...
                    : 'bg-blue-50 hover:bg-blue-105 dark:bg-blue-955/20 dark:hover:bg-blue-900/30 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-900/50'
                    }`}
                >
                  <Upload size={13} />
                  Approve
          ...
          <div className="flex flex-wrap items-center gap-3 bg-slate-50 dark:bg-slate-900/40 p-3.5 rounded-2xl border border-slate-200/60 dark:border-slate-800/80">
            ...
            <div className="flex-1 min-w-[200px] relative">
              <input
                ...
                onKeyDown={(e) => e.key === 'Enter' && handleApplyFilter()}
                className="w-full pl-3.5 pr-3.5 py-2 text-xs bg-white dark:bg-slate-955 border border-slate-200 dark:border-slate-850 rounded-xl focus:outline-none focus:ring-1 focus:ring-blue-500 text-slate-700 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 shadow-sm"
              />
            </div>
            <div className="flex items-center gap-2">
              <button
          ...
          {showUploadArea && (
            <div className="bg-slate-50 dark:bg-slate-955/10 border-2 border-dashed border-slate-205 dark:border-slate-800 rounded-2xl p-6 transition-all animate-in slide-in-from-top-4 duration-300">
              ...
              <div className="flex flex-col items-center justify-center p-6 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 rounded-2xl shadow-sm">
                ...
                <div className="flex items-center gap-3 mt-5">
                  <button
                    ...
                    className="flex items-center gap-1.5 bg-slate-105 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-750 text-slate-650 dark:text-slate-300 text-xs font-bold rounded-xl border border-slate-200 dark:border-slate-755 transition-colors shadow-sm"
                  >
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
            {loading && shares.length === 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 text-slate-400 px-4">
                ...
                <p className="text-xs text-slate-505 mt-1">Start by clicking "Share" button.</p>
              </div>
            ) : (
                ...
                <table className="w-full text-left border-collapse table-fixed min-w-[1400px]">
                  <thead>
                    <tr className="border-b border-slate-150 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-950/25">
                      ...
                      <th className="w-40 py-3 px-4 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400 text-center sticky left-[48px] z-20 bg-slate-50 dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800">Actions</th>
                      <th className="w-16 py-3 px-3 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400 text-center">S.No</th>
                      <th className="w-64 py-3 px-3 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400">Product Name</th>
                      <th className="w-48 py-3 px-3 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400">SKU</th>
                      <th className="w-40 py-3 px-3 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400">Price Type</th>
                      <th className="w-44 py-3 px-3 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400">Supplier SKU</th>
                      <th className="w-28 py-3 px-3 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400 text-right">Min Order Qty</th>
                      <th className="w-28 py-3 px-3 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400 text-right">Max Capacity</th>
                      <th className="w-24 py-3 px-3 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400 text-right">BufStock</th>
                      <th className="w-20 py-3 px-3 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400 text-right">LTD</th>
                      <th className="w-32 py-3 px-3 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400 text-right">Supplier Price</th>
                      <th className="w-24 py-3 px-3 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400 text-center">Preferred</th>
                      <th className="w-32 py-3 px-3 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400">Valid From</th>
                      <th className="w-32 py-3 px-3 text-xs font-bold uppercase tracking-wider text-slate-505 dark:text-slate-400">Valid To</th>
                    </tr>
                  </thead>
                    ...
                    {shares.map((shareItem, index) => {
                      ...
                      return (
                        ...
                        >
                            ...
                            <div className="flex items-center justify-center gap-1">
                              {subTab === 'DRAFT' && (
                                <button
                                  ...
                                  className="p-1.5 rounded-lg text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 dark:hover:text-emerald-400 dark:hover:bg-emerald-955/30 transition-all"
                                  title="Approve"
                                >
                                  <Check size={14} />
                                </button>
                              ...
                              {(subTab === 'DRAFT' || subTab === 'APPROVED') && (
                                <button
                                  ...
                                  className="p-1.5 rounded-lg text-slate-505 hover:text-amber-600 hover:bg-amber-50 dark:hover:text-amber-400 dark:hover:bg-amber-955/30 transition-all"
                                  title="Block"
                                >
                                  <Slash size={14} />
                                </button>
                          ...
                          <td className="py-2.5 px-3 text-xs text-slate-705 dark:text-slate-300 text-center font-semibold">
                            {shareItem.isPreferred ? (
                              <span className="px-2 py-0.5 rounded-full text-[10px] bg-emerald-50 text-emerald-600 dark:bg-emerald-950/20 dark:text-emerald-400">Yes</span>
                            ) : (
                              <span className="px-2 py-0.5 rounded-full text-[10px] bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400 font-normal">No</span>
                            )}
                          </td>
                          <td className="py-2.5 px-3 text-xs text-slate-600 dark:text-slate-400 truncate">
                            {shareItem.contractValidFrom ? shareItem.contractValidFrom.split('T')[0] : (shareItem.validFrom ? shareItem.validFrom.split('T')[0] : 'N/A')}
                          </td>
                          <td className="py-2.5 px-3 text-xs text-slate-600 dark:text-slate-400 truncate">
                            {shareItem.contractValidTo ? shareItem.contractValidTo.split('T')[0] : (shareItem.validTo ? shareItem.validTo.split('T')[0] : 'N/A')}
                          </td>
                        </tr>
                      );
                    })}
            ...
            <div className="bg-slate-50 dark:bg-slate-955/10 border-2 border-dashed border-slate-205 dark:border-slate-800 rounded-2xl p-6 transition-all animate-in slide-in-from-top-4 duration-300">
              <div className="flex items-center justify-between mb-4">
                ...
                <button
                  ...
                  className="px-2.5 py-1 text-slate-505 hover:text-slate-700 dark:hover:text-slate-350 text-xs font-semibold hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-all"
                >
                  Cancel
                </button>
              </div>
                ...
                <div className="flex items-center gap-3 mt-4">
                  ...
                  {selectedFile && (
                    <button
                      onClick={handleUploadExcel}
                      disabled={uploadingExcel}
                      className="flex items-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white text-xs font-bold rounded-xl transition-all shadow-sm"
                    >
          ...
          <div className="flex flex-wrap items-center gap-3 bg-slate-50 dark:bg-slate-900/40 p-3.5 rounded-2xl border border-slate-200/60 dark:border-slate-800/80">
            <div className="flex-1 min-w-[200px] relative">
              <input
                ...
                className="w-full pl-3.5 pr-3.5 py-2 text-xs bg-white dark:bg-slate-955 border border-slate-200 dark:border-slate-850 rounded-xl focus:outline-none focus:ring-1 focus:ring-blue-500 text-slate-700 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-505 shadow-sm"
              />
            </div>
            <div className="flex-1 min-w-[200px] relative">
              <input
            ...
            <div className="flex items-center gap-2">
              ...
              {(appliedHistorySku || appliedHistoryProductName || historySkuSearch || historyProductNameSearch) && (
                <button
                  ...
                  className="px-3.5 py-2 border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-505 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 text-xs font-bold rounded-xl transition-all shadow-sm"
                >
                  Clear
                </button>
              )}
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
            {historyLoading ? (
              ...
              <div className="flex flex-col items-center justify-center p-12 text-center">
                ...
                <h3 className="text-sm font-bold text-slate-707 dark:text-slate-300">No Price Changes Found</h3>
      ...
      {deleteConfirmOpen && (
        <ConfirmDeleteModal
          ...
          message={
            deleteTarget === 'single'
              ? 'Are you sure you want to delete/deactivate this supplier product share?'
              : `Are you sure you want to delete/deactivate the selected ${selectedIds.length} supplier product shares?`
          }
        />
      )}
