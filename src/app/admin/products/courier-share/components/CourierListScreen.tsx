...
interface CourierListScreenProps {
...
export default function CourierListScreen({
  ...
  const fetchClients = useCallback(async () => {
    ...
    setError('');
    try {
      const { data } = await axiosInstance.get<any>('/vendor/suppliers', {
      ...
      console.error(err);
      setError('Failed to load suppliers. Please check API server or try again.');
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
            ...
            Supplier Product Sharing
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            Select a supplier to view and manage shared products, pricing structures, and history.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800/80 p-4 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        ...
        <div className="relative w-full md:flex-1">
          ...
          <input
            ...
            className="w-full h-10 pl-9 pr-4 text-xs bg-slate-50/50 hover:bg-slate-50 dark:bg-slate-955 border border-slate-200 hover:border-slate-300 dark:border-slate-800 focus:border-blue-500 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 text-slate-800 dark:text-slate-200 placeholder:text-slate-400 dark:placeholder:text-slate-505 transition-all font-medium font-sans"
          />
        </div>
        <button
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-2xl shadow-sm overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 text-slate-500">
            ...
            <p className="text-sm font-medium">Fetching supplier records...</p>
          </div>
        ) : error ? (
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[700px]">
              <thead>
                <tr className="border-b border-slate-150 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-950/25">
                  ...
                  <th className="w-48 py-3 px-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 select-none">Supplier Code</th>
                  <th className="py-3 px-4 text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 select-none">Supplier Name</th>
                ...
                {paginated.map((client, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-4 text-xs font-semibold text-slate-800 dark:text-slate-205 truncate">
                        {client.clientName}
                      </td>
                      <td className="py-2.5 px-4 text-xs font-medium text-slate-605 dark:text-slate-400 truncate">
                        {client.legalName}
                      </td>
